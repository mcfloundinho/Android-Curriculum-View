 ...
 */
public class CurriculumMonthLoader implements CurriculumViewLoader {
    private MonthChangeListener mOnMonthChangeListener;

    public CurriculumMonthLoader(MonthChangeListener listener){
        this.mOnMonthChangeListener = listener;
    }

    @Override
    public double toCurriculumViewPeriodIndex(Calendar instance){
        return instance.get(Calendar.YEAR) * 12 + instance.get(Calendar.MONTH) + (instance.get(Calendar.DAY_OF_MONTH) - 1) / 30.0;
    }

    @Override
    public List<? extends ClassEvent> onLoad(int periodIndex){
    ...
    public interface MonthChangeListener {
         ...
         */
        List<? extends ClassEvent> onMonthChange(int newYear, int newMonth);
    }
}
